...
class CountAggregator(Aggregator):
    ...
    def aggregate(self, graph, node_index, conditional_node_to_label_map):
        neighbor_undirected = []
        ...
        if self.directed:
            ...
            for i in graph.neighbors(node_index):
                #TODO: use semisupervised method to get neighbor node lables
                if i in conditional_node_to_label_map:
...
class LocalClassifier(Classifier):
    ...
    def fit(self, graph, train_indices, feature_matrix):
    
        feature_list = []
        label_list = []

        for i in train_indices:
            feature_list.append( feature_matrix[i] )
            label_list.append(graph.node[i]['label'])
    ...
    def predict(self, graph, test_indices, feature_matrix):
    ...
    def predict_proba(self, graph, test_indices, feature_matrix):
        feature_list = []

        for i in test_indices:
            feature_list.append( feature_matrix[i] )
        
        return self.clf.predict_proba(feature_list)
...
class RelationalClassifier(Classifier):
    ...
    def fit(self, graph, train_indices, feature_list, conditional_node_to_label_map):
        '''
        ...
        labels = [graph.node[i]['label'] for i in train_indices]

        features = []
        for i in train_indices:
            self.feature_combination_check(graph, features, i, conditional_node_to_label_map, feature_list)

        self.clf.fit(features, labels)
    
    def predict(self, graph, test_indices, conditional_node_to_label_map, feature_matrix):
        '''
            This function should be called only after the fit function is called.
            Predict the labels of test Nodes conditioning on the labels in conditional_node_to_label_map.
        ...
        features = []
        for i in test_indices:
            self.feature_combination_check(graph,features,i,conditional_node_to_label_map, feature_matrix)
        return self.clf.predict(features)
    
    def feature_combination_check(self,graph,features,i,conditional_map, feature_matrix):
        aggregates = self.aggregator.aggregate(graph, i, conditional_map)
        feat_list = np.array([])
        if self.use_node_attributes:
            feat_list = np.array( [feature_matrix[i]] )
...
class ICA(Classifier):
    ...
    def fit(self, graph, train_indices, feature_list, conditional_node_to_label_map):
        self.local_classifier.fit(graph, train_indices, feature_list)
        self.relational_classifier.fit(graph, train_indices, feature_list, conditional_node_to_label_map)
    ...
    def predict(self, graph, test_indices, conditional_node_to_label_map, feature_matrix):
        '''
            ...
            '''
        predictclf = self.local_classifier.predict(graph, test_indices, feature_matrix)
        ...
        for eachTrail in range(self.max_iteration):
            for x in test_indices:
                temp.append(x)
                rltn_pred=list(self.relational_classifier.predict(graph,temp,conditional_node_to_label_map, feature_matrix))
